...
def getdata():
    data = timeTB.query.all()
    result = []
    for row in data:
        result.append({'stime': row.stime, 'time': row.time, 'venue':row.venue, 'course':row.course, 'c_color':row.c_color, 'whichday':row.whichday, 'faculty':row.faculty})
